...
class GenerationContext: ObservableObject {
    ...
    @Published var previews: Double = Settings.shared.previewCount
    @Published var disableSafety = false
    @Published var previewImage: CGImage? = nil

    @Published var computeUnits: ComputeUnits = (Settings.shared.userSelectedComputeUnits ?? .cpuAndGPU)
    ...
    func generate(prompt: String? = nil) async throws -> GenerationResult {
        guard let pipeline = pipeline else { fatalError("couldnt get pipeline") }
...
class Settings {
    ...
    private init() {
        defaults.register(defaults: [
            Keys.model.rawValue: "defaultModelId",
